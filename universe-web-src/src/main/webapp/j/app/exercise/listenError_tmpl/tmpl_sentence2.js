...
define(function(){
  ...
  '<div class="right-part1">',
    ...
    '<audio src="" id="audioAudio">亲 您的浏览器不支持html5的audio标签</audio>',
      '<p class="bold mleft25"><img src="../../i/i19.png"><span class="word-position family-STHeitiSC">{{data.errWord.question.prompt}}</span><span class="right hear-percent font12 family-yahei">{{data.errWord.question.sequence_number}}/{{data.errWord.question.group_count}}</span></p>',
      /*'<ul class="hear-ul">',
      '{{each data.errWord.question.simpleChoices as value index}}',
        '<li>',
          '<span class="glyphicon choicehear glyphicon-ok hear-back pointer choiceErrSentence2" data-choice="{{index}}"></span>',
...
*/
   '<table class="table1 mleft25 tingli-table" cellpadding="0" cellspacing="0">',
    '{{each data.errWord.question.simpleChoices as value index}}',      
      '<tr>',
        '<td class="td1 word-layout2"><span class="glyphicon glyphicon-ok choicehear voca-back pointer choiceErrSentence2" data-choice="{{index}}"></span></td>',
