...
define(['common/uniqueAjax', 'common/render', 'xml2json', 'app/baseURL', 'common/valiEnter', 'baseCookie', 'lib/store', 'audioControl'], function(uniqueAjax, Render, xml2json, URL, ValiEnter, BaseCookie) {
	var _conf,
		...
		dicMsg = "",      //20161116
		TMPL = { 
			t1: 'app/exercise/tmpl_dictation_submit',
			t2: 'app/exercise/tmpl_dictation_unit',
			t3: 'app/exercise/tmpl_dictation_login',
			result: 'app/exercise/tmpl_dictation_result'
		};
...
//		$document.on('trigger_side3', '#side3', side3);
		...
		$document.on('click', '#nextSub', nextSubject);
		$document.on('click', '#selectUnit', selectUnit);
		$document.on('click', '.someUnit', someUnit);

		$document.on('click', '#dictation-repeat', doRepeat); //再做一遍
		$document.on('click', '#dictation-onlywrong', doOnlyWrong); //重做错题
...
//				var dictation_results = new Array(userDoneRecords.length);
				...
				date=null;
				$.ajax({
					url: URL.xiaomaURL + 'dictation/results',
					type: 'POST',
	...
	var showDictation = function(groupId,planDayId,exerciseId,title) {
		...
		getDataMaster({
			url: URL.xiaomaURL + 'dictation/group/'+ curGroupId,
			type: 'GET',
			async: false,
	...
	function buildDictation(index){	
			//计算需要生成多少个输入框
				...
				dicMsg['stem'] = dicMsg.questions[index].stem; //20170823 添加题干字段
				...
				renderMaster({
					...
					cbk: function() {
						//初始化播放器
						if(dicMsg.audioUrl && dicMsg.audioUrl.length > 1) initAudioControl('myMusic');
						if (markWrongIng) {
							//做错题不显示计时
							$('.time-pos').hide();
							if (questionNum == questionCount) {
