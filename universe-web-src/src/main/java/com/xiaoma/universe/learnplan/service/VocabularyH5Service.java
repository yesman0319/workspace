...
@Service
public class VocabularyH5Service {
	...
	public Object getAll(HttpServletRequest request, int sequenceNumber) {
		Map<String ,String> headers = getHeaders(); 
		ResponseData data = ApiClient.get(PLAN_API_URL+"/vocabulary/group/"+sequenceNumber, headers,request,ApiClient.FORMAT_DEFAULT);
		Object wordVO = null;
		try {
			System.out.println(data.getBackData());
			JSONObject jb = JsonUtil.jsonToObject(data.getBackData());
			if(jb.containsKey("questions")){
				return jb.getJSONArray("questions");
			}
		} catch (Exception e) {
			wordVO = null; 
			e.printStackTrace();
