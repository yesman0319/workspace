...
/*       '<span><a href="/html/chongci.html" class="bold bread-gray">正在练习</a><span class="sign1 bread-gray">></span></span>',
  ...
  '{{if data.rate != 100}}',
   '<div class="two-button2">',
    '<button type="button" class="btn btn-primary btn9" data-question_id="{{data.question_id}}" id="againTpoListen">再练一遍</button>',
    '<button type="button" class="btn btn-primary btn2" data-question_id="{{data.question_id}}" data-pickArtical="true" id="errOnlyTpoListen">重做错题</button>',
   ...
   '</div>',
    ...
    '{{else}}',
     '<div class="two-button3">',
      '<button type="button" class="btn btn-primary btn9" data-question_id="{{data.question_id}}" id="againTpoListen">再来一遍</button>',
      //'{{if !data.is_last_question}}',
      //'<button type="button" class="btn btn-primary btn1" id="nextUnitListen" data-next_question_id="{{data.next_question_id}}"  data-next_group_name="{{data.next_group_name}}" data-next_type_name="{{data.next_type_name}}" data-next_rate="{{data.next_rate}}" data-is_last_question="{{data.is_last_question}}">下一单元</button>',
      //'{{/if}}',
      '{{if !data.isFromPlan}}',
