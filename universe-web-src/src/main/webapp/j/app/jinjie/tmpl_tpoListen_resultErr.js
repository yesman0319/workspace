...
/*       '<span><a href="/html/chongci.html" class="bold bread-gray">正在练习</a><span class="sign1 bread-gray">></span></span>',
   ...
   '<div class="hear-judge family-yahei">',
        ...
        '<span><span style="margin-right: 24px;">{{data.sectionTypeNum}}</span>  对：{{data.correctCount}}  错：{{data.errorCount}}</span>',
        '</div>',
   '{{/if}}',
   '</div>',
  ...
  '{{if data.errorCount != 0}}',
   '<div class="two-button2">',
    ...
    '<button type="button" class="btn btn-primary btn2" data-question_id="{{data.question_id}}" data-pickArtical="false" id="errOnlyTpoListenErr">重做错题</button>',
