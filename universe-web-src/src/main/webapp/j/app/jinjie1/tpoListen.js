...
define(['common/uniqueAjax', 'common/render', 'xml2json', 'app/baseURL', 'baseCookie', 'lib/store'], function(uniqueAjax, Render, xml2json, URL, BaseCookie) {
	...
	var initEvent = function() {
		$(document).on('trigger_side2', '#side2', function() {
			$('#side2').click()
		})
		$(document).on('click', '#side2', tpoListenUnit);
	...
	var tpoListenResult = function(param) {
		...
		renderData.sectionType = param.name;
	...
	// var translateToggleTpoListen = function() {
	...
	// var videosToggleTpoListen = function() {
	// 	if ($("#jiexiTip").css('display') != "none") {
	// 		$("#jiexiTip").css('display', 'none')
	// 		$(".showEnable").css('display', 'block')
	// 		$(".showDisable").css('display', 'none')
	...
	// var jiexiToggleTpoListen = function() {
	// 	if ($("#jiexiTip").css('display') != "none") {
	// 		$("#jiexiTip").css('display', 'none')
	// 		$(".showEnable").css('display', 'block')
	...
	var getTpoListen = function(param) { //得到artical,questions,vip_user
		...
		for (var i = 0; i < xml.questionContent.length; i++) {
			...
			//排序题型
			if (questionContent.questionType == 'sort') {
				var gArray = [];
			...
			//排序题型
			if (questionContent.questionType == 'directMultiple') {
		...
		if ($('#tpoListenJieXi').css('display') != 'none') {
			...
			if (questions[currentQuestionIndex].questionType == 'single') {
				...
				if (yourAnswer == questions[currentQuestionIndex].correctResponse) {
					$('#sortAnswerDiv').css('color', '#6bc04b')
					$('#sortTable').find('span.showDisable').addClass('tpo-right')
				} else {
					$('#sortAnswerDiv').css('color', '#fa3131')
