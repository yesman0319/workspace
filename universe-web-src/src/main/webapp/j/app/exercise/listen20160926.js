...
define(['common/uniqueAjax', 'common/render', 'xml2json', 'app/baseURL', 'baseCookie', 'lib/store'], function(uniqueAjax, Render, xml2json, URL, BaseCookie) {
	...
	var initEvent = function() {
		...
		$(document).on('click', '#sentence1Submit', function(e) {
			clearTimerListen(); //弹出登录层暂停时间
			$('#dialogLogin').modal({
				backdrop: 'static'
			})
		...
		$(document).on('click', '#sentence2Submit', function(e) {
			clearTimerListen(); //弹出登录层暂停时间
			$('#dialogLogin').modal({
				backdrop: 'static'
			})
	...
	var listenSentenceUnit1 = function() {
		$('#listenSentenceUnit').html('')
		var _success = function(json) {
			if($(".nav li.active").find("a").attr('id') != "listenSentenceTab1"){
				console.log("return");
				return;
			}
			//清空全局变量
	...
	var listenSentenceUnit2 = function() {
		$('#listenSentenceUnit').html('')
		var _success = function(json) {
			if($(".nav li.active").find("a").attr('id') != "listenSentenceTab2"){
				console.log("return");
				return;
			}
			//清空全局变量
	...
	var choiceWord1 = function(param) {
			...
			if (param.choice != correctResponse) {//做错题20160912
				record.isError = true;
				var errObj={};
				errObj.answer=param.choice==1?"A":param.choice==2?"B":param.choice==3?"C":"D";
				errObj.is_correct=2;
				...
				if (isEmpty(token)) { //用户没登陆，先弹框让其登录
					...
					$('#dialogLogin').bind('hidden.bs.modal', function(e) {
						...
						if (!isEmpty(BaseCookie.getToken())) {
							...
							$('#sentence2SubmitDiv').css('display', 'block')
							startTimerListen();
						}
					})
				} else { //出结果页
	...
	var submitWord1 = function(param) {
		...
		var digitRound = function(digit, length) {
			...
			$.ajax({
				...
				success: function(json) {
					//isRewardCoupon = json.is_reward_coupon;20160910
				}
			})
	...
	var resultWord1 = function(type, rate) {
			...
			if (type == 1) { //做题来的，直接从records和errids取
				...
				var _success = function(json) {
					...
					renderData.errWords = json.questions;
	...
	var resultErrSentence2Result = function() {
		...
		///**-------计算最高等级 开始-------**/
		//console.log("计算最高等级---->sentence2-result");
		//var groupLevelFromList=getGroupLevelFromList(groups[currentGroupIndex].id);
		//var highGroupLevel=groupLevel_fromError>groupLevelFromList ? groupLevel_fromError : groupLevelFromList ;
		//renderData.high_group_level=highGroupLevel;
		///**-------计算最高等级 结束-------**/
	...
	var showListen=function(groupId,planDayId,exerciseId,xmTitle){//listenWord()
		xm_groupId=groupId;
	...
	var gotoHisResult=function(groupId,planDayId,exerciseId,xmTitle){//hisListen
		xm_groupId=groupId;
		xm_planDayId=planDayId;
		xm_exerciseId=exerciseId;
		xm_title=xmTitle;
