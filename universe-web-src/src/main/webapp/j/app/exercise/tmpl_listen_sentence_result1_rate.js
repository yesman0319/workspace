...
define(function(){
  return [
   ...
   /* '<p class="p1 bold black mleft25"><span>练句>听音辨句>Unit<span id="">{{data.group_sequence_number}}</span>练习报告</span><span class="unit"><a href="#" id="sentenceUnit1">选择单元</a></span></p>',*/
      '<p class="p1 bold black mleft25"><img src="../../i/list-pic.png" style="margin-left:   -6px;margin-right:10px;">练习报告<span class="unit"></p>',
    '<hr class="mleft25" />',
    '<div class="family-song" >',
     ...
     /*  '<div class="i25">',
      ...
      '{{if data.rate == 100}}',
      '<div class="right-part2" style="right:0;">',
      ...
      '<nav class="pagination4 center">',
       '{{each data.errWords as value index}}',
        '<span class="page">',
         /*'<a href="##" rel="prev" {{if value.is_correct == 2}}class="bred"{{else}}class="ball-blue"{{/if}}>{{value.sequence_number}}</a>',*/
   ...
   '{{if data.rate != 100}}',
     '<div class="two-button2">',
      '<button type="button" class="btn btn-primary active btn9" data-volunit="1" data-volgroupid="1" id="againSentence1">再来一遍</button>',
      '<button type="button" class="btn btn-primary active btn2" data-group_level="{{data.group_level}}" data-volunit="1" data-volgroupid="1" id="errOnlySentence1Result">重做错题</button>',
      /*'<button type="button" class="btn btn-primary btn1 {{if data.canUnLocked!=1}}disabled{{/if}}" id="nextUnitSentence1">下一单元</button>',*/
      '{{if !data.isFromPlan}}',
      '<button type="button" class="btn btn-primary btn1" id="showMorePlan">完成</button>',
      '{{/if}}',
