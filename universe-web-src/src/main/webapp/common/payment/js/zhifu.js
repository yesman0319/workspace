...
window.onload=function(){
	...
	var order_price = document.getElementById("price").innerHTML;//订单价格
	var goodid = document.getElementById("goodid").value;
	var goodattrid = document.getElementById("goodattrid").value;
    ...
    for(var i=0;i<navA.length;i++){
        ...
        navA[i].onclick=function(){
            ...
            if(this.innerHTML=="支付宝"){
            	...
            	var resultCode = "";
            	$.ajax({
            	    url: "/orders/ali/web/dmf?goodid="+goodid+"&goodattrid="+goodattrid+"&orderid="+orderid, 
            	    type: "get", 
            	    dataType:"json",
            	    async:false, //设置成同步
            	    success: function (data) {
            	    	...
            	    	if(data.status==0){  //请求  支付宝 成功
                            ...
                            imgs.src="/common/payment/img/pic4.jpg";
                            divs.id="btns";
                            ...
                            h2.appendChild(span);
                            div.appendChild(h2);
                            img.src="/orders/qrcode?code_url="+resultCode; //当面付，扫码支付
                            ...
                            document.getElementById('btns').onclick=function(){
                            	...
                            	window.open(data.interfacehost+"/pay/ali/web/jsdz?goodid="+goodid+"&attrid="+goodattrid+"&orderid="+orderid);
                            } //即时到账   
...
function ajaxstatus() {
	var orderid = document.getElementById("orderid").value;//本地订单号
	var plan_card_order_id = document.getElementById("plan_card_order_id").value;//本地订单号
	...
	$.ajax({
	   // url: "/orders/result?orderId="+orderid,  //调用商城查询支付结果，现改为调用 计划卡片平台查询支付结果
    	url: "/orders/result?plan_card_order_id="+plan_card_order_id,
	    type: "get",
	    dataType:"json", 
	    success: function (data) {
	     	if(data.status==0){//支付成功
	     		window.clearInterval(timer);
	     		location.href="/orders/resultjsp?order_id="+orderid+"&goodid="+goodid+"&planid="+planid+"&order_price="+order_price+"&pay_result=success&pay_type="+paytype;
	     	}
	    }
	});
 }
	...
	$.ajax({
	    ...
	    success: function (data) {
	     	if(data.status==0){//支付成功
	     		...
	     		location.href="/orders/resultjsp?order_id="+orderid+"&goodid="+goodid+"&planid="+planid+"&order_price="+order_price+"&pay_result=error&pay_type="+paytype;
	     	} 
	    }
	});
}
...
fanhui.onclick=function(){
	var goodid = document.getElementById("goodid").value;
	var planid = document.getElementById("planid").value;
    location.href="/orders/get?goodid="+goodid+"&planid="+planid; 
}

/*距离当前时间45分钟后，失效，动态显示倒计时*/
var endTime= new Date();
    endTime = endTime.setTime(endTime.getTime()+45*60*1000);
function GetRTime(){
