'use strict'

define(function(){
  return [

  '<div class="right-part2 right-partTPO">',
    '{{if data.artical}}',
       /**面包屑部分**/
       '<p class="Ntpo_title Ntpo_border" id="tpoReadPart1">',
       '  <span><img src="../../i/list-pic.png" class="list-pic" style="margin-left:15px;margin-right:-10px;"><span class="bread-gray gray808 jinJieUnit pointer mleft25">{{data.xm_title}}</span></span>',
       //'  <span class="black23 ">{{data.tpoNum}} <span class="sign1 bread-gray">&gt;</span> Passage {{data.artical.sequence_number}}</span>',
       '</p>',

       /**题目部分**/
       '<div class="Ntpo_left Ntpo_border mright20 tpoPart2" id="tpoReadPart2"></div>',

       /**翻译部分**/
       '<div class="Ntpo_right Ntpo_border" id="tpoReadPart3">',
          '<div class="N_font_b text-right mright25 mbot15 N_c_p">',
             '<span class="N_tpoicon N_ticon_hidden" style="cursor: auto;"></span>',
             '<a class="all-blue font14" id="seeFanyi1">查看翻译</a>',
          '</div>',
          '<div class="Ntpo_right_p font14">',
             '{{each data.artical.p_en as value index}}',
              '<p>{{value}}</p>',
              '{{each data.artical.p_ch as val ind}}',
                '{{if index == ind}}',
                  '<p class="N_c_fy fanyi1" style="display: none;">{{val}}</p>',
                '{{/if}}',
              '{{/each}}',
             '{{/each}}',
          '</div>',
       '</div>',
    '{{/if}}',
  '</div> '
  ].join('')
})
