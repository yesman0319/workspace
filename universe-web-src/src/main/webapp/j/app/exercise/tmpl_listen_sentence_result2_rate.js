  ...
  return [
    ...
    '<div class="family-song" >',
      ...
      '<nav class="pagination4 center">',
       '{{each data.errWords as value index}}',
        '<span class="page">',
         '<a href="##" rel="prev" {{if value.is_correct == 1}}class="ball-blue"{{else}}class="bred"{{/if}}>{{value.question_sequence_number}}</a>',
       '</span>',
      '{{/each}}',
      '</nav>',
     '{{/if}}',
   '{{if data.rate != 100}}',
     '<div class="two-button2">',
      '<button type="button" class="btn btn-primary active btn9" data-volunit="1" data-volgroupid="1" id="againSentence2">再来一遍</button>',
      '<button type="button" class="btn btn-primary active btn2" data-group_level="{{data.group_level}}" data-volunit="1" data-volgroupid="1" id="errOnlySentence2Result">重做错题</button>',
      '<button type="button" class="btn btn-primary btn1 {{if data.canUnLocked!=1}}disabled{{/if}}" id="nextUnitSentence2">下一单元</button>',
      '</div>',
    '</div>',
   '{{/if}}',
