...
define(function(){
  return [
      ...
      '<p class="totel-unit">',
        '<a href="#" class="wordUnitDetailWord2" data-locked="{{value.locked}}" data-group_level="{{value.group_level}}" data-index="{{index}}" data-id="{{value.id}}" data-sequenceNumber="{{value.sequence_number}}"  data-questionCount="{{value.question_count}}" data-rate="{{value.rate}}" data-errorQuestionIds="{{value.error_question_ids}}">unit{{value.sequence_number}}{{if value.is_today_task}}<span class="orange left16">今日任务</span>{{/if}}',
        '</a>',
/*        '{{if value.rate}}',
          '{{if value.rate.split("/")[0] == value.rate.split("/")[1]}}',
           '<a class="green wordUnitDetail2 pointer" data-index="{{index}}" data-id="{{value.id}}" data-sequenceNumber="{{value.sequence_number}}"  data-questionCount="{{value.question_count}}" data-rate="{{value.rate}}" data-errorQuestionIds="{{value.error_question_ids}}">(正确率: 100%)</a>',
          '{{/if}}',
          '{{if value.rate.split("/")[0] != value.rate.split("/")[1]}}',
            // '<a class="green wordUnitDetail2 pointer" data-index="{{index}}" data-id="{{value.id}}" data-sequenceNumber="{{value.sequence_number}}"  data-questionCount="{{value.question_count}}" data-rate="{{value.rate}}" data-errorQuestionIds="{{value.error_question_ids}}">(对:{{value.rate.split("/")[0]}} 错:{{value.rate.split("/")[1]-value.rate.split("/")[0]}})</a>',
          '<a class="green wordUnitDetail2 pointer" data-index="{{index}}" data-id="{{value.id}}" data-sequenceNumber="{{value.sequence_number}}"  data-questionCount="{{value.question_count}}" data-rate="{{value.rate}}" data-errorQuestionIds="{{value.error_question_ids}}">(正确率: {{(value.rate.split("/")[0] / value.rate.split("/")[1] * 100).toString().split(".")[0]}}%)</a>',
          '{{/if}}',
        '{{/if}}',*/
      '{{if value.locked==0}}',
        '{{if value.rate}}',
            '{{if value.group_level == 0}}',
                '<img src="../../i/newimg-0.png" data-group_level="{{value.group_level}}" data-avg_speed="{{value.avg_speed}}" data-index="{{index}}" data-id="{{value.id}}" data-sequenceNumber="{{value.sequence_number}}"  data-questionCount="{{value.question_count}}" data-rate="{{value.rate}}" data-errorQuestionIds="{{value.error_question_ids}}" class="i25-6 wordUnitDetail2 pointer">',
              '{{/if}}',
              ...
              '{{/if}}',
          // '{{else}}',
          //     '<img src="../../i/i25-5.png" data-index="{{index}}" data-id="{{value.id}}" data-sequenceNumber="{{value.sequence_number}}"  data-questionCount="{{value.question_count}}" data-rate="{{value.rate}}" data-errorQuestionIds="{{value.error_question_ids}}" class="i25-6 wordUnitDetailWord2 pointer">',
          '{{/if}}',
      '{{else}}',
