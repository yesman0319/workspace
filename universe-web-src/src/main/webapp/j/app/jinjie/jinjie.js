...
define(['common/uniqueAjax', 'common/render', 'xml2json', 'app/baseURL', 'baseCookie', 'lib/store'], function(uniqueAjax, Render, xml2json, URL, BaseCookie) {
	...
	var initEvent = function() {
		...
		//$(document).on('trigger_side1_1', '#side1_1', function() {
		//	$("#side1_1").click();
		//});
		// $(document).on('click', '#side1', toDoRender);
		//$(document).on('click', '#side1_1', jinJieUnit);
		...
		$(document).on('click', '.multipleChoice', function(e) {
			if ($(e.target).hasClass('tpo-choice')) {
				$(e.target).removeClass('abc','tpo-choice')
			} else {
				$(e.target).addClass('abc','tpo-choice')
			}
		});
		...
		$(document).on('click', '.questionPageErrJinJie', function(e) {
			var data = {
				'pageNum': $(e.target).attr('data-pageNum'),
				'localIndex': $(e.target).attr('data-localIndex')
			};
			questionPageErrJinJie(data)
		});
		//题号面板,题号前面的面板
...
//			alert();
//			console.log($(".jinJieUnitDetail:first").html())
//			$(".jinJieUnitDetail:first").eq(0).trigger("click");
		}
		$.ajax({
	...
	var jinJieUnitDetail = function(param, callback) {
		localIndex = 0;
		//$("#jinJieMore").css("display", "none")20160919
		var _success = function(json) {
		...
		$.ajax({
			...
			url: URL.xiaomaURL + 'tpo/common/questions/'+param.questionId+'/'+xm_type,
			type: 'get',
			headers: {
	...
	var jinJieResult = function(param) {
		...
		$.ajax({
			url: URL.xiaomaURL + 'tpo/common/results/'+param.questionId+'/'+xm_type,
			type: 'get',
			headers: {
				"Authorization": exerciseApi.xiaomaToken
			},
	...
	var getJinJie = function(param) { //得到artical,questions,vip_user
		questions = [];
		artical = {};
		//var xml = $.xml2json(param.content).itemBody;//20160919
		artical.p_en = param.en_web;
		artical.p_ch = param.ch;
		for (var i = 0; i < param.tpo_question.length; i++) {
			...
			/*	1 single 2 multiple 3 sort 4 complex 5 directMultiple 6 insert*/
			question.questionType=typeTmp==1?"single":typeTmp==2?"multiple":typeTmp==3?"sort":typeTmp==4?"complex":typeTmp==5?"directMultiple":"insert";
			question.sequenceNumber =questionContent.section_number;//问题序号
			question.correctResponse = questionContent.answer.join();//问题答案
			question.p = questionContent.analysis;
			...
			if (question.questionType == 'complex') {
				...
				/*var tmpAnswer1=[],tmpAnswer2=[];
				$.each(questionContent.answer,function(index,value){
					if(value.indexOf('G1')>0){
						tmpAnswer1.push(value.substring(0,1));
					}else{
						tmpAnswer2.push(value.substring(0,1));
					}
				});

				G1.correctResponse=tmpAnswer1.join();
				G1.p=questionContent.G1;
				G2.correctResponse=tmpAnswer2.join();
				G2.p=questionContent.G2;*/
				G1.correctResponse=questionContent.answer1.join();
				G1.p=questionContent.G1;
				G2.correctResponse=questionContent.answer2.join();
				G2.p=questionContent.G2;
	...
	var questionPageErrJinJie = function(param) {
		//题号前面板进行保存
		// $('.tpo-right-part3').css('display', 'block')
	...
	//保存做题记录
	var saveChoice = function() {/*	1 single 2 multiple 3 sort 4 complex 5 directMultiple 6 insert*/
			playCount = 0;
			//记录该题记录
			if (questions[currentQuestionIndex].questionType == 'single') {
				var answer = "";
	...
	var againJinJie = function(param) {
		...
		if (isEmpty(param.questionId)) {
			param1 = {
				'questionId': artical.id //为空是，是从计算的结果页来，需从article中拿id就行
			};
		} else {
			param1 = {
    ...
    var showJinjie=function(questionId,planDayId,exerciseId,type,xmTitle){
