...
<div class="layout">
    <div class="main fl">
        <div class="h">${title}<span class="title">${name}</c:if></span><i class="share_icon"    param='${basePath}/h5/speakingswritings/${type }/${questionId}.html?uid=${uid}%26source=weixin%26medium=weixinfriend%26campaign=${shareTitle}<c:if test="${uid>0 }">%26campaignContent=uid${uid}</c:if>'></i></div>
        <div class="question">
            <h2>问题</h2>
