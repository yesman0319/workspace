...
define(function(){
  ...
  '<div class="main mid">',
    ...
    '</p>',
    '<hr class="mleft25">',
    '<p class="left25 sentence" id="sentence" content="{{data.content}}">',
  ...
  '<div class="one-button2">',
    '<button type="button" class="btn btn-primary btn1" id="doAgain">再练一次</button>',
   '</div>',
  '<div class="h202">',
   '<div>',
    '<p class="p1 bold center">我的练习记录</p>',
    '{{each data.answer_messages as value index}}',
    '<div>',
      '<hr class="mleft25" />',
      '<p class="p4 bold"><span class="correct-ans">{{value.answer_created_at.substring(0, 19).replace("T", "&nbsp;&nbsp;")}}</span>{{if value.type == 1}}<span class="red score-wcorrect">待批</span>{{else if value.type == 2}}<span class="green score-wcorrect">{{value.score}}分</span>{{/if}}</p>',
      '<p class="left25 sentence score-p6 right13">',
      '<a href="#" class="article_item" type="{{value.type}}" answer_id="{{value.answer_id}}" score="{{value.score}}">',
      '{{if value.answer_conent.length > 90}}{{value.answer_conent.substring(0, 90) + "...."}}{{else}}{{value.answer_conent}}{{/if}}',
      '</a>',
      '</p>',
     '</div>',
    '{{/each}}',
